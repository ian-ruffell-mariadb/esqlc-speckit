...
| Q9 | How does a unit declare **two SQLDAs of different `sqlvar-count`**? Example 10-1 parameterises the count — `} sqlvar[sqlvar-count];` — while Table 10-2 has `INCLUDE SQLDA` generate one type named `SQLDA_TYPE`, and §10 p.10-30 refers to `sizeof(struct SQLDA_TYPE)`. Those cannot all hold when a program declares an input descriptor and an output descriptor of different sizes, which §10's own dynamic-SQL flow needs. p.10-19 pairs a count-1 `dummy_da` with a malloc'ed descriptor, so the case is sidestepped rather than answered. | FR-007.6a, FR-007.26 | unresolved — Gate 10 **refuses** a second `INCLUDE SQLDA` whose count differs, rather than guessing. Emitting the type once at the first count silently truncates the second descriptor; emitting it twice is a redefinition error; a distinct tag per descriptor contradicts Table 10-2's single generated name. |
